...
# --------------------------------------------------------------- image uploads
#
# Shared by the profile photo and the two Business Card faces below — all
# three are just "one image file belonging to a contact", stored under
# Config.UPLOADS_DIR with a random filename (never the browser's original
# name, which could collide or carry path characters) and referenced from
# a single TEXT column on contacts (profile_image_path /
# business_card_front_path / business_card_back_path).

def _save_image_upload(file_storage):
    """Validate and save an uploaded image; return the stored filename (not
    the full path — that's reconstructed from Config.UPLOADS_DIR when
    serving) or None if no valid file was provided."""
    if not file_storage or not file_storage.filename:
        return None
    ext = file_storage.filename.rsplit(".", 1)[-1].lower() if "." in file_storage.filename else ""
    if ext not in Config.ALLOWED_IMAGE_EXTENSIONS:
        flash(f"Image must be one of: {', '.join(sorted(Config.ALLOWED_IMAGE_EXTENSIONS))}.", "error")
        return None
...
def _delete_image_upload(filename):
    if not filename:
        return
...
@contacts_bp.route("/<int:contact_id>/business-card/<side>")
@login_required
def contact_business_card(contact_id, side):
    """Serves the front or back Business Card image (side = "front" or
    "back") — mirrors contact_photo above."""
    if side not in ("front", "back"):
        abort(404)
    column = "business_card_front_path" if side == "front" else "business_card_back_path"
    db = get_db()
    contact = db.execute(
        f"SELECT {column} AS path FROM contacts WHERE contact_id = ? AND tenant_id = ?", (contact_id, g.tenant_id)
    ).fetchone()
    if contact is None or not contact["path"]:
        abort(404)
    return send_from_directory(Config.UPLOADS_DIR, contact["path"])
...
def new_contact():
    ...
    if request.method == "POST":
        ...
        photo_filename = _save_image_upload(request.files.get("profile_image"))
        front_filename = _save_image_upload(request.files.get("business_card_front"))
        back_filename = _save_image_upload(request.files.get("business_card_back"))
        if photo_filename or front_filename or back_filename:
            db.execute(
                "UPDATE contacts SET profile_image_path = COALESCE(?, profile_image_path), "
                "business_card_front_path = COALESCE(?, business_card_front_path), "
                "business_card_back_path = COALESCE(?, business_card_back_path) WHERE contact_id = ?",
                (photo_filename, front_filename, back_filename, contact_id),
            )
            db.commit()
...
def edit_contact(contact_id):
    ...
    if request.method == "POST":
        ...
        # Profile photo + the two Business Card faces are all "one image
        # belonging to this contact" (see _save_image_upload above) — same
        # replace-or-remove handling for each, just against a different
        # column/file-field/checkbox name.
        for field, column, remove_flag in [
            ("profile_image", "profile_image_path", "remove_photo"),
            ("business_card_front", "business_card_front_path", "remove_business_card_front"),
            ("business_card_back", "business_card_back_path", "remove_business_card_back"),
        ]:
            if form.get(remove_flag):
                _delete_image_upload(contact[column])
                db.execute(
                    f"UPDATE contacts SET {column} = NULL WHERE contact_id = ? AND tenant_id = ?",
                    (contact_id, g.tenant_id),
                )
            else:
                new_filename = _save_image_upload(request.files.get(field))
                if new_filename:
                    _delete_image_upload(contact[column])
                    db.execute(
                        f"UPDATE contacts SET {column} = ? WHERE contact_id = ? AND tenant_id = ?",
                        (new_filename, contact_id, g.tenant_id),
                    )
