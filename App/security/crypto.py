...
Split into two independent concerns for multi-tenant, multi-user GSS (see
schema.sql MODULE T for the full rationale):
  ...
  * Per-TENANT field encryption is handled here. Each tenant gets its own
    ...
    created (see new_tenant_dek() below). As of this draft there are no
    encrypted columns in GSS's schema (Platforms & Subscriptions and
    Personal Accounts, PIMS's two encrypted-field modules, are both out of
    scope — see GSS_schema_v1.sql's header), but the encrypt_value()/
    decrypt_value() utilities below stay in place for whatever sensitive
    column comes next.
