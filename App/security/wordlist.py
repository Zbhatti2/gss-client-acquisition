"""
Recovery-phrase wordlist and generator (password recovery via seed words).

NOTE: this is a locally-defined, human-friendly wordlist for this app's
account recovery only. It is deliberately NOT the standard BIP-39 wordlist
used by cryptocurrency wallets — there is no need for cross-wallet
interoperability here, only for a phrase a person can legibly write down
and re-type. 220 words * 12 picks (with repetition allowed) gives
~12 * log2(220) ≈ 104 bits of entropy, comfortably enough for account
recovery.
"""
import hashlib
import secrets
