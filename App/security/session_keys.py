...
This intentionally does NOT put the DEK in the Flask session cookie: the
cookie is signed (tamper-evident) but not encrypted, so its contents are
readable by anyone with the cookie — not where a field encryption key
should live.
...
keyed by a hashed token, or an OS keyring) at that point.
"""
import threading
import time
